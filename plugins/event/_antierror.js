...
const usersData = Object.values(global.db.users);
const groupsData = Object.values(global.db.groups);
...
if (isNaN(user.limit)) user.limit = setting.limit.free;
});

groupsData.forEach(group => {
if (group.sewa && isNaN(group.sewa.expired)) {
group.sewa.expired = 0;
group.sewa.status = false;
group.sewa.notice = false;
}
if (group.sewa && group.sewa.status && group.sewa.expired == 0) {
group.sewa.expired = 0;
group.sewa.status = false;
group.sewa.notice = false;
}
