...
mecha.reply(m.chat, `Jangan tag dia!\nDia sedang AFK ${reason ? 'dengan alasan ' + reason : ''}\nSelama *${func.clockString(new Date - afkTime)}*`, m).then(async () => {
//mecha.reply(jid, `Seseorang dari grup *${await (await mecha.groupMetadata(m.chat)).subject}*, menandai atau menyebut Anda.\n\nNama: @${m.sender.split('@')[0]}\nPesan: ${m.budy}`, m)
})
}
}
},
