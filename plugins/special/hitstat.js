...
let caption = `乂  *H I T - S T A T*\n\n`
caption += `*“Total command hit statistics are currently ${Object.entries(global.db.statistic).map((v) => v[1].hit).reduce((a, b) => a + b)} Hits.”*\n`
caption += sorted.slice(0, show).map(([name, v], i) => `\n${i + 1}. *Command* :  ${m.prefix + name}
    *Hit* : ${v.hit}
